...
function job(persona, band, index, title, required, preferred = []) {
  ...
  return {
    ...
    description_text: [
      title,
      "About the role",
      `You will work as a ${title}.`,
      "Requirements",
      ...required.map((skill) => `Must have ${skill}.`),
      ...preferred.map((skill) => `Nice to have ${skill}.`),
      "Benefits",
      ...
      "Legal",
      "This posting is not a contract of employment. Employment is at will.",
      `Posting ${id}.`,
    ].join("\n"),
